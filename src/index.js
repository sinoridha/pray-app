...
setInterval(function() {
  // console.log('[condition]', today.format('YYYY-MM-DD') == moment().format('YYYY-MM-DD'));
  if (today.format('YYYY-MM-DD') != moment().format('YYYY-MM-DD')) {
    console.log('[setScedule] true');
    today = moment();
    setScedule();
  } else {
    console.log('[setScedule] false');
  }
}, 60 * 1000);

function setScedule() {
	console.log('setScedule() run')
  const times = getPrayTime();
  const ftimes = formatingDate(times);
  setNotificationSchedule('2018-01-12 14:16:30', "Pray Time1 !", "It's fajr, let pray.");
